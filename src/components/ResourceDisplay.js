...
const ResourceDisplay = () => {
    ...
    const elements = [
        {
            title:`Food: ${food.reduce((total, item) => total + item.amount, 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`,
            content:'Food is used to feed settlers and earned when killing cattle. Food will expire after around 3 days.'
        },
        {
            title:`Preserved Food: ${preservedFood.reduce((total, item) => total + item.amount, 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`,
